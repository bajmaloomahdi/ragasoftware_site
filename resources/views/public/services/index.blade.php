...
@section('content')
    ...
    <div class="container-site section-pad">
        <div class="max-w-2xl">
            <h1 class="text-3xl font-extrabold text-ink-900 sm:text-4xl">خدمات راگا سافت‌ور</h1>
            ...
            <div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($services as $service)
                    <a href="{{ route('services.show', $service->slug) }}" class="card card-hover group flex flex-col">
                        ...
                        <h2 class="text-lg font-bold text-ink-900 group-hover:text-brand-600">{{ $service->title }}</h2>
                        <p class="mt-2 flex-1 text-sm leading-7 text-ink-600">{{ \Illuminate\Support\Str::limit($service->summary, 130) }}</p>
                    </a>
                @endforeach
            </div>
        @endif
