    ...
    <article class="section-pad">
        <div class="container-site grid gap-12 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <h1 class="text-3xl font-extrabold text-ink-900 sm:text-4xl">{{ $service->title }}</h1>
                    ...
                    <ul class="mt-8 space-y-3">
                        @foreach($service->features as $f)
                            <li class="flex gap-3">
                                ...
                                <div>
                                    <p class="font-semibold text-ink-900">{{ $f->title }}</p>
                                    @if($f->description)<p class="text-sm text-ink-600">{{ $f->description }}</p>@endif
                                </div>
                            </li>
                        @endforeach
                    </ul>
            ...
            <aside class="lg:col-span-1">
                <div class="sticky top-24 rounded-2xl border border-paper-200 bg-paper-50 p-6">
                    <h2 class="text-base font-bold text-ink-900">درخواست این خدمت</h2>
                    <p class="mt-2 text-sm text-ink-600">برای دریافت مشاوره و برآورد، با ما تماس بگیرید.</p>
    ...
    @if($related->isNotEmpty())
        <section class="section-pad bg-paper-50">
            <div class="container-site">
                <h2 class="mb-8 text-xl font-bold text-ink-900">سایر خدمات</h2>
                <div class="grid gap-6 sm:grid-cols-3">
                    @foreach($related as $item)
                        <a href="{{ route('services.show', $item->slug) }}" class="card card-hover">
                            <h3 class="font-bold text-ink-900">{{ $item->title }}</h3>
                            <p class="mt-2 text-sm text-ink-600">{{ \Illuminate\Support\Str::limit($item->summary, 90) }}</p>
                        </a>
                    @endforeach
                </div>
            </div>
