...
@section('content')
    ...
    <div class="container-site section-pad">
        <div class="max-w-2xl">
            <h1 class="text-3xl font-extrabold text-ink-900 sm:text-4xl">نمونه‌کارها و پروژه‌ها</h1>
            <p class="mt-4 lead">پروژه‌هایی که با راگا سافت‌ور به نتیجه رسیده‌اند.</p>
        </div>

        @if($projects->isEmpty())
            <p class="mt-12 text-ink-500">به‌زودی…</p>
        @else
            <div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($projects as $project)
                    <a href="{{ route('projects.show', $project->slug) }}" class="group overflow-hidden rounded-2xl border border-paper-200 bg-white">
                        ...
                        <div class="p-5">
                            ...
                            <h2 class="mt-1 font-bold text-ink-900">{{ $project->title }}</h2>
                            @if($project->summary)<p class="mt-2 text-sm text-ink-600">{{ \Illuminate\Support\Str::limit($project->summary, 90) }}</p>@endif
                        </div>
                    </a>
                @endforeach
            </div>
