...
@section('content')
    ...
    <article class="section-pad">
        <div class="container-site max-w-3xl">
            ...
            <h1 class="mt-2 text-3xl font-extrabold text-ink-900 sm:text-4xl">{{ $project->title }}</h1>
            ...
            @if($project->completed_on || $project->project_url)
                <dl class="mt-6 flex flex-wrap gap-x-10 gap-y-2 border-y border-paper-200 py-4 text-sm">
                    @if($project->completed_on)
                        <div><dt class="text-ink-400">تاریخ اجرا</dt><dd class="font-medium text-ink-800">{{ jdate($project->completed_on, 'F Y') }}</dd></div>
                    @endif
                    @if($project->project_url)
                        <div><dt class="text-ink-400">لینک</dt><dd><a href="{{ $project->project_url }}" target="_blank" rel="noopener" class="font-medium text-brand-600" dir="ltr">{{ $project->project_url }}</a></dd></div>
                    @endif
                </dl>
            @endif
    ...
    @if($related->isNotEmpty())
        <section class="section-pad bg-paper-50">
            <div class="container-site">
                <h2 class="mb-8 text-xl font-bold text-ink-900">پروژه‌های دیگر</h2>
                <div class="grid gap-6 sm:grid-cols-3">
                    @foreach($related as $item)
                        <a href="{{ route('projects.show', $item->slug) }}" class="group overflow-hidden rounded-2xl border border-paper-200 bg-white">
                            <x-picture :media="$item->cover" ratio="4/3" class="w-full object-cover" />
                            <div class="p-4"><h3 class="font-bold text-ink-900">{{ $item->title }}</h3></div>
                        </a>
                    @endforeach
                </div>
            </div>
