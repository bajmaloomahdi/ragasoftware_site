...
@section('content')
    ...
    <div class="container-site section-pad max-w-3xl">
        <h1 class="text-3xl font-extrabold text-navy-900 sm:text-4xl">سؤالات متداول</h1>
        <p class="mt-4 lead">پاسخ پرسش‌های رایج درباره محصولات، پیاده‌سازی و پشتیبانی راگا سافت‌ور.</p>

        @php($allFaqs = collect())

        @foreach($categories as $category)
            <h2 class="mt-12 text-xl font-bold text-navy-900">{{ $category->name }}</h2>
            <div class="mt-4 divide-y divide-mist-200 border-y border-mist-200" x-data="{ open: null }">
                @foreach($category->faqs as $faq)
                    @php($allFaqs->push($faq))
                    <div>
                        <button type="button" @click="open === {{ $faq->id }} ? open = null : open = {{ $faq->id }}"
                                class="flex w-full items-center justify-between gap-4 py-5 text-right text-base font-semibold text-navy-900" :aria-expanded="open === {{ $faq->id }}">
        ...
        @if($uncategorised->isNotEmpty())
            <div class="mt-12 divide-y divide-mist-200 border-y border-mist-200" x-data="{ open: null }">
        ...
        <div class="mt-14 rounded-2xl bg-mist-50 p-8 text-center">
            <p class="font-bold text-navy-900">پاسخ سؤال‌تان را پیدا نکردید؟</p>
